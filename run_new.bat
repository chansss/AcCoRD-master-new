...
set "exe=bin\accord_win.exe"
set "configDir=config"
set "resultsDir=results"
set "outDir=matlab_new"
...
for %%F in ("%configDir%\accord_config_sample*.txt") do (
  ...
  set "cfgPath=%%~fF"
  set "cfgBase=%%~nF"
  set "logPath=%TEMP%\accord_new_!cfgBase!_!RANDOM!!RANDOM!.log"

  echo === Running NEW: !cfgBase! (SEED=1) ===
...
echo All NEW runs completed. Results are in "%outDir%".
echo.
echo Press any key to exit...
pause >nul
